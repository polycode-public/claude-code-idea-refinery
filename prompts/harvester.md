...
Read `ideas-operator-rejected/REASONS.md` when it exists: a find that plainly matches
a standing rejection pattern is dropped, and a borderline match gets the pattern named
in the seed's first comment so the scorer weighs it.

A find must pass the charter's three gates before it becomes a seed, and the seed's own
...
the same value proposition, do not seed a duplicate: drop the find, or if the new signal
