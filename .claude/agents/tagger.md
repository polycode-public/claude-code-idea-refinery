...
clustering deterministically, and add the one thing a script cannot: judgment. You name
what themes.mjs found, stamp that name onto the ideas that share it, and write
`THEMES.md` so the rest of the refinery — and the overseer — can see the shape the
corpus is taking.
